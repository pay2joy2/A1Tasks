...
public class Task3 {
    ...
    public static void main(String[] args) throws SQLException, IOException, CsvValidationException {
        ...
        /**
         *
         * Для первой загрузки, необходимо раскомментировать обе функции, и запустить через данный PSVM
         * последующие запуски - через Tomcat
         *
         * Скрипты SQL для создания баз данных находятся в task3.postgres.ddl
         * Первая загрузка logins.csv в базу данных
         *
         * Для корректной проверки был добавлен тестовый сотрудник, аккаунт которого есть в postings.csv
         *
         */
         ...
         *
         * Для корректной проверки была добавлена новая запись, текущего года, обращение через Postman c period = Year;
         *
         *
         */
